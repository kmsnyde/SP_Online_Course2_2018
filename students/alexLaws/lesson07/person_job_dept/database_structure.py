...
class Job(BaseModel):
    ...
    time_in_job = IntegerField()
    logger.info('Number')
    salary = DecimalField(max_digits=7, decimal_places=2)
    logger.info('Which person had the Job')
    person_employed = ForeignKeyField(Person, related_name='was_filled_by', null=False)
    logger.info('In which department is the job')
    department_id = ForeignKeyField(Department, related_name='was in this department', null=False)
