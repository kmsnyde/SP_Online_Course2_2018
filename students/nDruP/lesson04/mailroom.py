...
def save_to_dir(save_name, save_text):
    ...
    if save_txt == 'y':
        write_dir = input_dir()
        if write_dir:
            return write_str_to_file(save_name, save_text, write_dir)
    return ""


def write_str_to_file(f_name, content, wrt_dir=os.getcwd(), f_type='txt'):
    """
    ...
    file_name = f_name.replace(' ', '_') + "_" + curdate + '.' + f_type
...
def mr_exit():
    ...
    if save_confirm == 'y':
        print(write_str_to_file("dict_init", d.to_json(), os.getcwd(), '.json'))
    sys.exit()
