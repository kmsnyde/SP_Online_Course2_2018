...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        on the exact boundary of the margin region,
        ...
        :param actions: a dictionary containing the keys
        'PUMP_IN', 'PUMP_OFF', and 'PUMP_OUT'

        :return: The new action for the pump:
        one of actions['PUMP_IN'], actions['PUMP_OUT'], actions['PUMP_OFF']
        """

        low_marg = self.target_height * (1-self.margin)
        high_marg = self.target_height * (1+self.margin)

        next_action = current_action

        if current_action == actions['PUMP_OFF']:
            if current_height < low_marg:
                next_action = actions['PUMP_IN']
            elif current_height > high_marg:
