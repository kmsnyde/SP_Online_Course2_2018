...
class Decider:
    ...
    def decide(self, current_height, current_action, actions):
        ...
        :return: The new action for the pump: one of
                ...
                actions['PUMP_OFF']
        """
        upperb = self.target_height * (1 + self.margin)
        lowerb = self.target_height * (1 - self.margin)

        if current_action == actions['PUMP_OFF']:
            if current_height < lowerb:
                pump_next = actions['PUMP_IN']
            elif current_height > upperb:
                pump_next = actions['PUMP_OUT']
            else:
                pump_next = actions['PUMP_OFF']
