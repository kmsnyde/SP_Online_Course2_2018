...
class Controller:
    ...
    def tick(self):
        ...
        """
        liquid_hgt = self.sensor.measure()
        pump_curr = self.pump.get_state()
        pump_next = self.decider.decide(liquid_hgt, pump_curr, self.actions)
        return self.pump.set_state(pump_next)
