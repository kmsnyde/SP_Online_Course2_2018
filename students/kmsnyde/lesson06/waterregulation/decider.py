...
class Decider(object):
    ...
    def __init__(self, target_height, margin):
        ...
        self.above_margin = self.target_height * (1 + self.margin)
        self.below_margin = self.target_height * (1 - self.margin)
    ...
    def decide(self, current_height, current_action, actions):
        ...
        if current_action == actions['PUMP_OFF']:
            if current_height < self.below_margin:
                state = actions['PUMP_IN']
            elif current_height > self.above_margin:
                state = actions['PUMP_OUT']
            else:
                state = actions['PUMP_OFF']
